...
const settings = reactive([
  {
    ...
    value: computed({
      ...
      set: (val: boolean) => {
        settingStore.setMessageSoundEnabled(val)
      }
    })
  },
  {
  ...
  },
  {
    ...
    value: computed({
      get: () => {
        // 确保返回有效的主题值，如果为空则返回默认值
        const content = settingStore.themes.content
        return content || ThemeEnum.LIGHT
      },
      set: (val: string) => {
  ...
  },
  {
    ...
    type: 'select',
    value: computed({
      get: () => settingStore.page.lang || 'AUTO',
      set: (v: string) => {
        // 使用 $patch 确保响应式更新
