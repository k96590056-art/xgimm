<template>
  ...
  <n-config-provider
    ...
    <n-flex
      ...
      :size="8"
      justify="center">
      <span>Copyright {{ currentYear - 1 }}-{{ currentYear }} XG-IM All Rights Reserved.</span>
    </n-flex>
...
const register = async () => {
  ...
  try {
    ...
    // 注册（后端现在返回 LoginResultVO，包含 token 信息）
    const loginResult = await ImRequestUtils.register({
      ...info,
      deviceType: 'PC',
      clientId: clientId
    })
    ...
    await invoke(TauriCommand.UPDATE_TOKEN, {
      req: {
        uid: String(loginResult.uid),
        token: loginResult.token,
        refreshToken: loginResult.refreshToken
      }
    })

    // 保存 token 到 localStorage（用于前端使用）
    ...
    window.$message.error(error?.message || t('auth.register.messages.register_fail'))
  } finally {
