...
use crate::{
    pojo::common::{ApiResult, HttpErrorResponse},
...
impl ImRequestClient {
    ...
    >(
        ...
        loop {
            ...
            // 检查 HTTP 状态码，处理服务器错误响应
            if status.is_server_error() || status.is_client_error() {
                // 尝试解析为 Spring Boot 错误格式
                if let Ok(http_error) = serde_json::from_str::<HttpErrorResponse>(&response_text) {
                    let error_msg = http_error.get_message();
                    error!("HTTP Error {} for {}: {}", status.as_u16(), &url, error_msg);
                    return Err(anyhow::anyhow!("服务器错误 ({}): {}", status.as_u16(), error_msg));
                }
                // 无法解析错误响应，返回原始信息
                error!("HTTP Error {} for {}: {}", status.as_u16(), &url,
                       &response_text.chars().take(200).collect::<String>());
                return Err(anyhow::anyhow!("服务器错误 ({})", status.as_u16()));
            }
            ...
            let result: ApiResult<T> = match serde_json::from_str(&response_text) {
                Ok(r) => r,
                Err(e) => {
                    error!("Failed to parse response for {}: {}. Response body: {}",
                           &url, e, &response_text.chars().take(500).collect::<String>());
