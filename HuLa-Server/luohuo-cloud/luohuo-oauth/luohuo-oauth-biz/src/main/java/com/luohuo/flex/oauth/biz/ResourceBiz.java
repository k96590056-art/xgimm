...
public class ResourceBiz {
    ...
    /**
     * 查询当前用户可用的资源
     * 支持未登录用户访问：employeeId为null时返回公共资源（is_general=1）
     *
    ...
    public List<String> findVisibleResource(Long employeeId, Long applicationId) {
        ...
        if (employeeId == null || employeeId <= 0) {
            list = defResourceService.list(Wraps.<DefResource>lbQ()
                    .eq(DefResource::getIsGeneral, true)
                    .eq(DefResource::getState, true)
                    .eq(applicationId != null, DefResource::getApplicationId, applicationId)
                    .orderByAsc(DefResource::getSortValue));
     ...
     * <p>
     * 1. 租户管理员拥有指定应用的所有 资源
     * 2. 还没成为员工的用户，返回公共资源（is_general=1）
     * 3. 未登录用户（employeeId为null）返回公共资源
     *
     * @param applicationId 应用id
     * @return 资源树
     */
    public List<VueRouter> findVisibleRouter(Long applicationId, Long employeeId, String subGroup, ClientTypeEnum type) {
        ...
        if (employeeId == null || employeeId <= 0) {
            list = defResourceService.list(Wraps.<DefResource>lbQ()
                    .eq(DefResource::getIsGeneral, true)
                    .eq(DefResource::getState, true)
                    .in(DefResource::getResourceType, menuCodes)
                    .eq(applicationId != null, DefResource::getApplicationId, applicationId)
                    .orderByAsc(DefResource::getSortValue));
        } else {
            boolean isAdmin = baseRoleService.checkRole(employeeId, RoleConstant.TENANT_ADMIN);
